...
const CardItem = (props) => {
  ...
  const onAddOverlay = async (obj)=>{
    try{
      ...
      if(findOverlay){
        axios.delete(`https://64438b8a90738aa7c072e7a2.mockapi.io/cart/${findOverlay.id}`)
        props.setOverlayItems((over) => over.filter(o=>o.myId !== obj.myId))
      }
      else{
        const {data} = await axios.post('https://64438b8a90738aa7c072e7a2.mockapi.io/cart', obj)
  ...
  const onAddFav = async (obj)=>{
    try{
      ...
      if(findFavorites){
        axios.delete(`https://64439d2090738aa7c0742e2b.mockapi.io/favorites/${findFavorites.id}`)
        props.setFavorites((over) => over.filter(o=>o.myId !== obj.myId))
      }
      else{
        const {data} = await axios.post('https://64439d2090738aa7c0742e2b.mockapi.io/favorites', obj)
